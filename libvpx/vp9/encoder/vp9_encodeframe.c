...
static void cnn_pick_partition(VP9_COMP *cpi, ThreadData *td,
                              ...
                              int mi_row, int mi_col, BLOCK_SIZE bsize,
                              RD_COST *rd_cost, int64_t best_rd,
                              PC_TREE *pc_tree, uint8_t* cnn_partition, uint8_t flag) {
    VP9_COMMON *const cm = &cpi->common;
    TileInfo *const tile_info = &tile_data->tile_info;
    MACROBLOCK *const x = &td->mb;
    RD_COST dummy_rdc;
            ...
            for (int j = 0; j < 4; j++) {
                ...
                if(ordered_partition[index]==3) {
                    ...
                    for (int l = 0; l < 4; l++) {
                        p0_row = l / 2;
                        p0_col = l % 2;
                        ordered_partition[index] = p0[4*p2_row+2*p1_row+p0_row][4*p2_col+2*p1_col+p0_col];
                        index=index+1;
                    }
                }
                else
...
static void encode_rd_sb_row(VP9_COMP *cpi, ThreadData *td,
                             ...
                             TOKENEXTRA **tp,uint8_t partition[][85], int batch_size) {
  VP9_COMMON *const cm = &cpi->common;
  TileInfo *const tile_info = &tile_data->tile_info;
  MACROBLOCK *const x = &td->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  void** ptr;
  int reduced_batch_size=floor((float)cm->width/64)*floor((float)cm->height/64);
    ...
    if (sf->partition_search_type == FIXED_PARTITION || seg_skip) {
      ...
      if(batch_index<reduced_batch_size) {
          ...
          cnn_pick_partition(cpi, td, tile_data, tp, mi_row, mi_col, BLOCK_64X64,
                             &dummy_rdc, INT64_MAX, td->pc_root, cnn_partition, 0);
      }
      else
...
void vp9_encode_tile(VP9_COMP *cpi, ThreadData *td, int tile_row,
                     int tile_col){    VP9_COMMON *const cm = &cpi->common;
    const int tile_cols = 1 << cm->log2_tile_cols;
    ...
    for (mi_row = mi_row_start; mi_row < mi_row_end; mi_row += MI_BLOCK_SIZE) {
        for (mi_col = mi_col_start; mi_col < mi_col_end; mi_col += MI_BLOCK_SIZE) {
            if (!cpi->sf.use_nonrd_pick_mode){
                set_offsets(cpi, tile_info, x, mi_row, mi_col, BLOCK_64X64); // for correct reconstruction
                uint8_t *pix=x->plane[0].src.buf;
                int stride = x->plane[0].src.stride;
                for(int i=0;i<64;i++)
                    qp1_batch[64*batch_ind+i]=x->q_index;
  ...
  if(first_step==1)
    {
      ...
      run_output_tensors_all = (TF_Tensor**)malloc(batch_size*4 * sizeof(TF_Tensor*));
    ...
    run_inputs_tensors[0] = TF_NewTensor(TF_FLOAT, raw_input_dims, 4, image_batch,
                                         batch_size* 64*64 * sizeof(float), &deallocator, NULL);
    run_inputs_tensors[1] =  TF_NewTensor(TF_FLOAT, qp1_dims, 4, qp1_batch,
                                          batch_size*64 * sizeof(float), &deallocator, NULL);


    // Output 1 (size=1)
    float *raw_output_data1 = (float *) malloc(batch_size*4 * sizeof(float));
    run_output_tensors_all[0] = TF_NewTensor(TF_FLOAT, raw_output_dims1, 2, raw_output_data1,
                                             batch_size*4 * sizeof(float), &deallocator, NULL);

    // Output 2 (size=4)
    float *raw_output_data2 = (float *) malloc(batch_size*4*4 * sizeof(float));
    run_output_tensors_all[1] = TF_NewTensor(TF_FLOAT, raw_output_dims2, 2, raw_output_data2,
                                             batch_size*4*4 * sizeof(float), &deallocator, NULL);


    // Output 3 (size=16)
    float *raw_output_data3 = (float *) malloc(batch_size*16*4 * sizeof(float));
    run_output_tensors_all[2] = TF_NewTensor(TF_FLOAT, raw_output_dims3, 2, raw_output_data3,
                                             batch_size*16*4 * sizeof(float), &deallocator, NULL);

    // Output 4 (size=64)
    float *raw_output_data4 = (float *) malloc(batch_size*64*4 * sizeof(float));
    run_output_tensors_all[3] = TF_NewTensor(TF_FLOAT, raw_output_dims4, 2, raw_output_data4,
...
//    printf("Model infererence time %f\n",(double)(t1 - t0) / CLOCKS_PER_SEC);
    ...
    batch_size=ceil(((float)mi_row_end-mi_row_start)/MI_BLOCK_SIZE)*ceil(((float)mi_col_end-mi_col_start)/MI_BLOCK_SIZE);
