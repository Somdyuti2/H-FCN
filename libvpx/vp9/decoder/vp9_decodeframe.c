...
static void decode_partition(TileWorkerData *twd, VP9Decoder *const pbi,
  ...
  if(bsize==12)
  {
	  if(cm->show_frame==1){
	  	  if(partition==3){
	  	      fprintf(blockstats_file, "\n %d \t \t %d \t \t %d \t \t %d", n%num_superblocks+1,bsize, partition, cm->base_qindex);}
	        else{
	      	  fprintf(blockstats_file, "\n %d \t \t %d \t \t %d", n%num_superblocks+1,bsize, partition);}}
	  n++;
  }
  else{
	  if(cm->show_frame==1)
...
static const uint8_t *decode_tiles(VP9Decoder *pbi, const uint8_t *data,
  ...
  if(cm->show_frame==1){
    fprintf(blockstats_file,"Superblock # \t Block Size \t Partition \t Q Index \t Block Type");}
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    TileInfo tile;
